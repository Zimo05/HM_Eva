...
class EventTokenizer:
    ...
    def _pad(
        ...
        # Optional task-specific supervision masks (for example MIMIC) follow
        # the five standard model inputs and are padded with False.
        for mask_name in ('event_loss_mask', 'type_loss_mask', 'time_loss_mask'):
            if mask_name in encoded_inputs:
                batch_output[mask_name] = self.make_pad_sequence(
                    encoded_inputs[mask_name], False,
                    padding_side=self.padding_side,
                    max_len=max_length,
                    dtype=bool,
                )
