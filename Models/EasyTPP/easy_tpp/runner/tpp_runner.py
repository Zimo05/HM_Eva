...
class TPPRunner(Runner):
    ...
    def run_one_epoch(self, data_loader, phase):
        ...
        loglike_component_sums = OrderedDict()
        if phase in [RunnerPhase.TRAIN, RunnerPhase.VALIDATE]:
            for batch in data_loader:
                ...
                if len(batch_mask) == 3:
                    epoch_mask.append((batch_mask[0],))
                    epoch_time_mask.append((batch_mask[1],))
                    epoch_type_mask.append((batch_mask[2],))
                else:
                    epoch_mask.append(batch_mask)
                components = getattr(
                    self.model, 'last_loglike_components', None
                )
                if components:
                    for name, value in components.items():
                        loglike_component_sums[name] = (
                            loglike_component_sums.get(name, 0.0) + float(value)
                        )

            avg_loss = total_loss / total_num_event

            metrics_dict.update({'loglike': -avg_loss, 'num_events': total_num_event})
            if loglike_component_sums:
                for name, value in loglike_component_sums.items():
                    metric_name = name[:-4] if name.endswith('_sum') else name
                    metrics_dict[metric_name] = value / total_num_event
        ...
        if pred_exists and label_exists:
            if epoch_time_mask and epoch_type_mask:
                time_mask = concat_element(epoch_time_mask, False)[0].astype(bool)
                type_mask = concat_element(epoch_type_mask, False)[0].astype(bool)
                time_metrics = self.metric_functions(
                    epoch_pred, epoch_label, seq_mask=time_mask
                )
                type_metrics = self.metric_functions(
                    epoch_pred, epoch_label, seq_mask=type_mask
                )
                ...
                predictions_type = epoch_pred[1]
                for predicate_id in getattr(
                        self.model, 'target_predicate_ids', ()):
                    predicate_mask = np.logical_and(
                        type_mask, labels_type // 2 == predicate_id
                    )
                    if predicate_mask.any():
                        metrics_dict[
                            'target_{}_accuracy'.format(predicate_id)
                        ] = float(np.mean(
                            predictions_type[predicate_mask]
                            == labels_type[predicate_mask]
                        ))
            else:
                metrics_dict.update(self.metric_functions(
