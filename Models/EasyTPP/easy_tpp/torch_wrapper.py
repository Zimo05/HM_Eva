...
class TorchModelWrapper:
    ...
    def run_batch(self, batch, phase):
        ...
        batch = list(batch.to(self.device).values())
        if phase in (RunnerPhase.TRAIN, RunnerPhase.VALIDATE):
            # set mode to train
            is_training = (phase == RunnerPhase.TRAIN)
            self.model.train(is_training)

            # FullyNN needs time derivatives in the validation stage.
            ...
            if is_training:
                ...
                if self.model.event_sampler:
                    self.model.eval()
                    # FullyNN obtains intensities by differentiating its
                    # cumulative-hazard network with respect to time.  That
                    # derivative is also required during thinning-based
                    # validation predictions, so no_grad cannot be used for
                    # this model.
                    prediction_grad_flag = self.model_id == 'FullyNN'
                    with torch.set_grad_enabled(prediction_grad_flag):
                        if batch[1] is not None and batch[2] is not None:
                            label_dtime, label_type = batch[1][:, 1:].cpu().numpy(), batch[2][:, 1:].cpu().numpy()
                        if len(batch) >= 8:
                            event_mask = batch[5][:, 1:].cpu().numpy()
                            type_mask = batch[6][:, 1:].cpu().numpy()
                            time_mask = batch[7][:, 1:].cpu().numpy()
                            mask = (event_mask, time_mask, type_mask)
                        elif batch[3] is not None:
            ...
            if isinstance(mask, tuple):
                return loss.item(), num_event, (pred_dtime, pred_type), \
                    (label_dtime, label_type), mask
            return loss.item(), num_event, (pred_dtime, pred_type), \
                (label_dtime, label_type), (mask,)
        else:
