...
def _configure_evaluation_frontier(inference):
    """Optionally force the same checkpoint to evaluate every stored leaf."""
    if os.environ.get("HM_EVAL_ALL_LEAF") != "1":
        return inference
    leaf_count = len(inference.tree.leaf_ids)
    config = inference.tree.frontier_routing.config
    config.frontier_budget = max(2, leaf_count)
    config.frontier_min_experts = max(2, leaf_count)
    config.expansion_compute_cost = 0.0
    inference.tree.frontier_routing._sync_topology()
    return inference
