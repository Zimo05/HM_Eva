...
Training-time validation follows the same separation.  The read-only
`semantic_only` and `full_frozen` views share one prepared prefix batch and run
through `run_sequence_batch_compact()` (default `--validation-batch-size 64`),
so validation does not materialize event rows on every epoch.  Controller-only
and Write-only calibration still use sequential online rollouts because
Memory Bank/probation state is order-dependent.  Validation reads an isolated
in-memory checkpoint payload; `last.pt` is written once after the best-model
decision, while event-level diagnostics are regenerated by final evaluation.
