...
# stage boundary.  They are parsed with a None default below so a resume can
...
def _checkpoint_config_value(payload: Mapping, dest: str):
    ...
    if not all(
        ...
        for section in (training, wake, sleep, structure, model, frontier)
    ):
        return None
    ...
    if dest in {
        ...
        return training.get(
            "controller_target_version"
            if dest == "controller_target_version"
            else dest
        )
    if dest in {
        "z_dim", "node_dim", "memory_key_dim", "num_basis",
...
def _cl_config_value(payload: Mapping, dest: str):
    ...
    if dest in {
        ...
        if dest in training:
            return training[dest]
        optimizer = payload.get("optimizer", {})
        if isinstance(optimizer, Mapping) and dest in optimizer:
    ...
    for section_name in ("wake", "sleep", "structure", "training"):
        ...
        if isinstance(section, Mapping) and dest in section:
            return section[dest]
    if dest in _MERGE_ARG_TO_CHECKPOINT:
...
def _resolve_cli_values(
    args,
    *,
    resume_payload: Mapping | None,
    cl_config: Mapping | None,
) -> None:
    ...
    if isinstance(saved_training, Mapping) and "seed" in saved_training:
...
def _start_cl_stage(trainer, args, resume_payload: Mapping | None) -> None:
    """Reset task-local validation selection when a new CL task starts."""
...
def _parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Write C_init, the complete model state captured before any "
            "task-0 gradient update or Hawkes cold-start."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Structured committed Split/Merge/Prune events in JSONL. "
            "Defaults to topology_events.jsonl beside the checkpoint."
        ),
    )
    parser.add_argument(
...
def _apply_evaluation_ablation(trainer, name: str) -> None:
    """Apply a native training ablation after the trainer is constructed."""
    ...
    if name == "no_working":
        ...
        trainer.controller.set_calibration_thresholds(
            retrieve=1.01,
            adapt=trainer.controller.calibration_dict()["adapt_threshold"],
            write=1.01,
        )
    elif name == "no_sleep":
...
def main() -> None:
    ...
    if args.cl_task_id is not None:
        ...
        if args.cl_config is None:
            raise RuntimeError(
                "continual stages require --cl-config"
            )
    ...
    if args.split_manifest is not None:
        ...
        if args.split == "train":
            ...
            print(
                f"[Controller validation] sequences={len(validation_dataset)} "
    ...
    if args.controller_base_checkpoint is not None:
        ...
        trainer.training_config.topology_events_path = (
            args.topology_events_path
        )
    ...
    # C_init is defined before Hawkes cold-start.  The complete checkpoint is
    # serialized only after the encoder/trainer exist, so retain exact tensor
    # snapshots and temporarily restore them at that serialization boundary.
    initial_hawkes_state = None
    initial_tree_state = None
    initial_tree_metadata = None
    initial_tree_provenance = None
    if args.hawkes_checkpoint is None and args.initial_checkpoint_output:
        initial_hawkes_state = copy.deepcopy(hawkes.state_dict())
        initial_tree_state = copy.deepcopy(tree.state_dict())
        initial_tree_metadata = copy.deepcopy(
            getattr(tree, "initialization_metadata", {})
        )
        initial_tree_provenance = copy.deepcopy(
            getattr(tree, "data_provenance", {"evaluation_regime": "transductive"})
        )
    if args.hawkes_checkpoint is None and args.cold_start_epochs > 0:
        hawkes_init_checkpoint = Path(args.checkpoint).with_suffix(
            ".hawkes_init.pt"
        )
        hawkes_init_checkpoint.parent.mkdir(parents=True, exist_ok=True)
        hawkes.cold_start(
            dataset,
    ...
    _apply_cl_metadata(trainer, args)
    if (
        args.initial_checkpoint_output
        and initial_hawkes_state is not None
        and initial_tree_state is not None
        and initial_tree_metadata is not None
        and initial_tree_provenance is not None
        and args.resume is None
    ):
        post_hawkes_state = copy.deepcopy(hawkes.state_dict())
        post_tree_state = copy.deepcopy(tree.state_dict())
        post_tree_metadata = copy.deepcopy(
            getattr(tree, "initialization_metadata", {})
        )
        post_tree_provenance = copy.deepcopy(
            getattr(tree, "data_provenance", {"evaluation_regime": "transductive"})
        )
        hawkes.load_state_dict(initial_hawkes_state)
        tree.load_state_dict(initial_tree_state)
        tree.initialization_metadata = copy.deepcopy(initial_tree_metadata)
        tree.data_provenance = copy.deepcopy(initial_tree_provenance)
        trainer._reconcile_optimizer_parameters()
        trainer.save_checkpoint(args.initial_checkpoint_output, epoch=0)
        hawkes.load_state_dict(post_hawkes_state)
        tree.load_state_dict(post_tree_state)
        tree.initialization_metadata = post_tree_metadata
        tree.data_provenance = post_tree_provenance
        trainer._reconcile_optimizer_parameters()
        print(
            "[FWT] wrote C_init before task-0 Hawkes cold-start: "
            f"{args.initial_checkpoint_output}"
        )
    _apply_evaluation_ablation(trainer, args.evaluation_ablation)
    trainer.train(dataset, validation_dataset=validation_dataset)
