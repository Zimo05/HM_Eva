...
class TrainingLoopMixin:
    ...
    def _validate_controller_checkpoint(
        ...
        Write-only controller selection additionally pairs its online-write
        rows with a sequential read-only ``full_frozen`` reference.  The
        scalar frozen metric still uses the compact pass, but the paired rows
        must use the same reduction/order contract as the sequential online
        rollout; otherwise harmless GPU reduction-order noise can move a
        controller constraint across its threshold.
        ...
        for start in range(0, len(dataset), batch_size):
            ...
            full_results = full_inference.run_sequence_batch_compact(
                prepared,
                frontier_static_cache=static_cache,
                # Write-only event rows are regenerated by the sequential
                # reference pass below.  Avoid copying compact rows to CPU
                # here; the compact pass remains the scalar fast path.
        ...
        if capture_frozen_rows or expected_frozen_sha:
            # The immutable controller-only contract and its paired online
            # metrics historically use rows from the scalar evaluator.
            # Compact kernels are parity-tested numerically, but their GPU
            # reduction order need not reproduce those rows byte-for-byte.
            frozen_reference_inference = make_inference(InferenceConfig(
                adapt_working_memory=True,
                allow_memory_writes=False,
                update_memory_usage=False,
            ))
            frozen_rows = []
            for sequence in dataset:
                result = frozen_reference_inference.run_sequence(sequence)
                frozen_rows.extend(
        ...
        if write_only:
            ...
            if len(frozen) != len(online_rows) or any(
                (base["source_index"], base["event_index"])
                != (target["source_index"], target["event_index"])
                for base, target in zip(frozen, online_rows)
            ):
                raise RuntimeError(
                    "sequential full_frozen and full_online rows are not "
                    "aligned"
                )
            gains = [
    ...
    def _iter_masked_wavefront_batches(
        ...
        """Precompute stateless prefix data for an ordered Wake chunk."""
        chunk_size = max(
