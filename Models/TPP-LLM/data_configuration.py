...
class DataConfiguration:
    ...
    def retweet(self, output_dir=None, type_labels=None):
        return self._standard("retweet", output_dir, type_labels)

    def taxi(self, output_dir=None, type_labels=None):
        return self._standard("taxi", output_dir, type_labels)

    def stackoverflow(self, output_dir=None, type_labels=None):
        return self._standard("stackoverflow", output_dir, type_labels)

    def taobao(self, output_dir=None, type_labels=None):
        return self._standard("taobao", output_dir, type_labels)

    def amazon(self, output_dir=None, type_labels=None):
        return self._standard("amazon", output_dir, type_labels)

    def covid_policy_tracker(self, output_dir=None, type_labels=None, epsilon=1e-4):
    ...
    def dws(self, output_dir=None, variants=None, type_labels=None, epsilon=1e-6):
        ...
        for variant, (_, splits) in load_all_dws(
            ...
            outputs[variant] = self._write_splits("dws", splits, target, type_labels)
        return outputs

    def _standard(self, dataset_name, output_dir, type_labels):
        ...
        return self._write_splits(dataset_name, splits, target, type_labels)

    def _write_splits(self, dataset_name, splits, target, type_labels):
        outputs = {}
        for split, records in splits.items():
            ...
            for record in records:
                converted = {
                    ...
                    "type_text": event_texts(dataset_name, record["type_event"], type_labels),
                }
                if "cluster" in record:
                    converted["cluster"] = record["cluster"]
