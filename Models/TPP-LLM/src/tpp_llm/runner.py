...
class TPPLLMRunner(object):
    ...
    def run(
        ...
        num_train_epochs: int = 1, warmup_ratio: float = 0,
        ...
        final_test = self.run_epoch(dataloader_test, phase='eval') if dataloader_test else None
        return {'best_epoch': best_epoch, 'validation': metrics_val_best, 'test': final_test}

    @torch.no_grad()
    def event_predictions(self, dataloader: DataLoader) -> List[dict]:
        """Return the common event-level evaluation contract."""
        self.model.eval()
        output = []
        sequence_id = 0
        for raw_batch in tqdm(dataloader):
            batch = {
                'time_since_start': [x.to(self.device) for x in raw_batch['time_since_start']],
                'time_since_last_event': [x.to(self.device) for x in raw_batch['time_since_last_event']],
                'type_event': [x.to(self.device) for x in raw_batch['type_event']],
                'type_text': raw_batch['type_text'],
            }
            ...
            likelihoods = self.model.compute_log_likelihood(
                batch_event_time_deltas=batch['time_since_last_event'],
                batch_event_types=batch['type_event'],
                batch_hidden_states=hidden,
            )
            probabilities, predicted_times = self.model.predict_next_event_probs(hidden)
            for times, deltas, types, ll, probs, pred_times in zip(
                    batch['time_since_start'], batch['time_since_last_event'],
                    batch['type_event'], likelihoods, probabilities, predicted_times):
                for index in range(1, len(times)):
                    probability = probs[index - 1].detach().float().cpu()
                    predicted_delta = max(
                        float(pred_times[index - 1].detach().cpu())
                        - float(times[index - 1].detach().cpu()), 0.0
                    )
                    output.append({
                        'sequence_id': sequence_id,
                        'event_index': index,
                        'true_type': int(types[index].detach().cpu()),
                        'predicted_type': int(probability.argmax().item()),
                        'type_probabilities': probability.tolist(),
                        'true_delta_time': float(deltas[index].detach().cpu()),
                        'predicted_delta_time': predicted_delta,
                        'event_nll': -float(ll[index - 1].detach().cpu()),
                    })
                sequence_id += 1
        return output
